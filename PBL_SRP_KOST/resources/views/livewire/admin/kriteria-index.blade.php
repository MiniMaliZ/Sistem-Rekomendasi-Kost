<div style="display:flex; flex-direction:column; gap:1.5rem;">
    <div style="display:flex; align-items:flex-start; justify-content:space-between; flex-wrap:wrap; gap:1rem;">
        <div>
            <h1 style="margin:0; font-size:1.5rem; font-weight:800; color:#5D4D34;">Kriteria SPK</h1>
            <p style="margin:0.25rem 0 0; font-size:0.875rem; color:#7C929E;">Kelola kriteria untuk perhitungan AHP & SAW</p>
        </div>
        <button wire:click="openCreate" id="btn-tambah-kriteria" class="btn-primary-custom">
    ...
    <div style="position:relative;">
        ...
        <input wire:model.live.debounce.300ms="search" id="search-kriteria" type="text"
        ...
        <table style="width:100%; border-collapse:collapse;">
            <thead>
                <tr style="background-color:#5D4D34;">
                    ...
                    <th style="padding:0.75rem 1rem; text-align:left; font-size:0.7rem; font-weight:700; text-transform:uppercase; color:#DBD3C6;">Nama Kriteria</th>
                    <th style="padding:0.75rem 1rem; text-align:left; font-size:0.7rem; font-weight:700; text-transform:uppercase; color:#DBD3C6;">Dibuat oleh</th>
            ...
            <tbody>
                @forelse($kriterias as $kriteria)
                    <tr style="border-bottom:1px solid rgba(173,156,138,0.2);">
                        <td style="padding:0.75rem 1rem; font-size:0.875rem; color:#7C929E;">{{ $kriteria->id_kriteria }}</td>
                        <td style="padding:0.75rem 1rem; font-size:0.875rem; font-weight:600; color:#5D4D34;">{{ $kriteria->nama_kriteria }}</td>
                        <td style="padding:0.75rem 1rem; font-size:0.875rem; color:#7C929E;">{{ $kriteria->user->nama ?? '-' }}</td>
                        <td style="padding:0.75rem 1rem;">
                            <div style="display:flex; align-items:center; justify-content:center; gap:0.5rem;">
                                <button wire:click="openEdit({{ $kriteria->id_kriteria }})"
                                        style="display:inline-flex; align-items:center; justify-content:center; width:32px; height:32px; border-radius:8px; border:none; background:transparent; cursor:pointer;" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="#7C929E" stroke-width="2">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931z"/>
                                    </svg>
                                </button>
                                <button wire:click="confirmDelete({{ $kriteria->id_kriteria }})"
                                        style="display:inline-flex; align-items:center; justify-content:center; width:32px; height:32px; border-radius:8px; border:none; background:transparent; cursor:pointer;" title="Hapus">
                ...
                @empty
                    <tr>
                        <td colspan="4" style="padding:3rem 1rem; text-align:center; font-size:0.875rem; color:#7C929E;">Belum ada data kriteria.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if($showModal)
        <div style="position:fixed; inset:0; z-index:100; display:flex; align-items:center; justify-content:center; padding:1rem; background-color:rgba(0,0,0,0.5);">
            <div style="width:100%; max-width:420px; background-color:#DBD3C6; border:1px solid #AD9C8A; border-radius:1rem; box-shadow:0 25px 50px rgba(93,77,52,0.25);">
                <div style="display:flex; align-items:center; justify-content:space-between; padding:1.25rem 1.5rem; border-bottom:1px solid rgba(173,156,138,0.3);">
                    <h3 style="margin:0; font-size:1.1rem; font-weight:700; color:#5D4D34;">{{ $editingId ? 'Edit Kriteria' : 'Tambah Kriteria' }}</h3>
                ...
                <form wire:submit="save" style="padding:1.25rem 1.5rem; display:flex; flex-direction:column; gap:1rem;">
                    <div>
                        <label style="display:block; font-size:0.8rem; font-weight:600; color:#5D4D34; margin-bottom:0.375rem;">Nama Kriteria *</label>
                        <input wire:model="nama_kriteria" id="input-nama-kriteria" type="text" placeholder="Contoh: Harga, Jarak, Fasilitas"
                        ...
                        @error('nama_kriteria') <p style="font-size:0.75rem; color:#C45A5A; margin:0.25rem 0 0;">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label style="display:block; font-size:0.8rem; font-weight:600; color:#5D4D34; margin-bottom:0.375rem;">Dibuat oleh (User) *</label>
                        <select wire:model="id_user" id="input-id-user-kriteria"
                                style="width:100%; box-sizing:border-box; padding:0.5rem 0.875rem; background-color:#CDC6BA; border:1px solid #AD9C8A; border-radius:0.625rem; font-size:0.875rem; font-family:'Poppins',sans-serif; color:#5D4D34; outline:none;">
                        ...
                        @error('id_user') <p style="font-size:0.75rem; color:#C45A5A; margin:0.25rem 0 0;">{{ $message }}</p> @enderror
                    </div>
                    <div style="display:flex; justify-content:flex-end; gap:0.75rem; padding-top:0.75rem; border-top:1px solid rgba(173,156,138,0.3);">
                        ...
                        <button type="submit" id="btn-save-kriteria" class="btn-primary-custom">{{ $editingId ? 'Simpan' : 'Tambah' }}</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    @if($showDeleteModal)
        <div style="position:fixed; inset:0; z-index:100; display:flex; align-items:center; justify-content:center; padding:1rem; background-color:rgba(0,0,0,0.5);">
            <div style="width:100%; max-width:380px; background-color:#DBD3C6; border:1px solid #AD9C8A; border-radius:1rem; padding:2rem; text-align:center;">
                ...
                <h3 style="margin:0 0 0.5rem; font-size:1.1rem; font-weight:700; color:#5D4D34;">Hapus Kriteria?</h3>
                <p style="margin:0 0 1.5rem; font-size:0.875rem; color:#7C929E;">Kriteria ini akan dihapus beserta semua data terkait.</p>
                <div style="display:flex; gap:0.75rem; justify-content:center;">
                    <button wire:click="$set('showDeleteModal', false)"
                            style="padding:0.5rem 1.25rem; border-radius:0.625rem; font-size:0.875rem; font-weight:600; font-family:'Poppins',sans-serif; border:1px solid #AD9C8A; background:transparent; color:#5D4D34; cursor:pointer;">Batal</button>
                    <button wire:click="delete" id="btn-confirm-delete-kriteria" class="btn-danger-custom">Ya, Hapus</button>
                </div>
            </div>
        </div>
    @endif
