...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/user_listkost.css') }}">
</head>
    ...
    <div class="page-body">
        ...
        <section id="section-sidebar">
            ...
            <nav class="nav-pill">
                <ul class="nav-list">
                    <li>
                        <a href="{{ route('user.home') }}" class="nav-item" aria-label="Dashboard">
                            <x-tabler-layout-dashboard-filled class="nav-blade-icon" />
                        </a>
                    </li>
                    <li>
                        <a href="{{ route('user.kost') }}" class="nav-item nav-item--active" aria-label="Daftar Kost">
                            ...
                            <form id="filter-form" action="{{ route('user.kost') }}" method="GET">
                                ...
                                <div class="filters-row">
                                    ...
                                    <div class="filter-item">
                                        <div class="filter-toggle" data-filter="tipe">
                                            ...
                                            <img src="{{ asset('images/arrow-down.svg') }}" alt="Dropdown Arrow" class="icon-dropdown" />
                                        </div>
                                        ...
                                        <div class="filter-dropdown" id="tipe-dropdown" style="display: none;">
                                            @foreach($filterOptions['tipe_kos_options'] as $value => $label)
                                                <label class="filter-option">
                                                    <input type="checkbox" name="tipe_kos[]" value="{{ $value }}"
                                    ...
                                    <div class="filter-item">
                                            ...
                                            <div class="filter-text">
                                                ...
                                                <span class="filter-value">
                                                    @if(request('harga_min') || request('harga_max'))
                                                        ...
                                                        Rp. {{ number_format(request('harga_max', 999999999), 0, ',', '.') }}
                                                    @else
                                                        Semua Harga
                                                    @endif
                                                </span>
                                        ...
                                        <div class="filter-dropdown" id="harga-dropdown" style="display: none;">
                                            @foreach($filterOptions['harga_ranges'] as $label => $range)
                                                <label class="filter-option">
                                                    <input type="radio" name="harga_range" value="{{ $range['min'] }}-{{ $range['max'] }}"
                                ...
                                <div class="buttons-group">
                                    <a href="{{ route('user.kost') }}" class="btn-reset">
                                        <img src="{{ asset('images/reset.svg') }}" alt="Reset Icon" class="icon-reset" />
                                        <span>Reset</span>
                                    </a>
            ...
            <section id="results-header">
                <div class="container">
                    ...
                    <div class="sort-container">
                        ...
                        <form id="sort-form" action="{{ route('user.kost') }}" method="GET" style="display: inline;">
                            <!-- Preserve other query params -->
                            <input type="hidden" name="search" value="{{ request('search') }}">
                            <input type="hidden" name="tipe_kos" value="{{ implode(',', (array)request('tipe_kos', [])) }}">
                            <input type="hidden" name="harga_min" value="{{ request('harga_min') }}">
                            <input type="hidden" name="harga_max" value="{{ request('harga_max') }}">
                            
                            <select name="sort" class="sort-dropdown" onchange="document.getElementById('sort-form').submit()">
                                <option value="newest" @if($currentSort === 'newest') selected @endif>Terbaru</option>
                                <option value="price_asc" @if($currentSort === 'price_asc') selected @endif>Harga Terendah</option>
                                <option value="price_desc" @if($currentSort === 'price_desc') selected @endif>Harga Tertinggi</option>
                                <option value="name" @if($currentSort === 'name') selected @endif>Nama A-Z</option>
                            </select>
                        </form>
                    </div>
                </div>
                            ...
                            @foreach($kosts as $kost)
                                <div class="card" data-kost-id="{{ $kost->id_kost }}">
                                    <div class="card-image-wrapper">
                                        <img src="{{ $kost->link_original }}" alt="{{ $kost->nama_kost }}" class="card-image" />
                                        <div class="badge badge-{{ $kost->tipe_kos }}">
                                            {{ strtoupper($kost->tipe_kos) }}
                                        </div>
                                        <button class="btn-favorite fav-btn" 
                                                data-kost-id="{{ $kost->id_kost }}"
                                                data-is-favorit="{{ $kost->is_favorite ? 'true' : 'false' }}"
                                                aria-label="Favorite">
                                            <img src="{{ $kost->is_favorite ? asset('images/heart_filled.svg') : asset('images/heart_outline.svg') }}" alt="" />
                                        </button>
                                    </div>
                                        ...
                                        <div class="card-amenities">
                                            @php
                                                ...
                                                if (!empty($kost->fasilitas_kamar)) {
                                                    $amenities = array_slice(explode(',', $kost->fasilitas_kamar), 0, 3);
                                                }
                                                if (count($amenities) < 3 && !empty($kost->fasilitas_umum)) {
                                                    $amenities = array_merge($amenities, array_slice(explode(',', $kost->fasilitas_umum), 0, 3 - count($amenities)));
                                                }
                                            @endphp
            ...
            @if($kosts->hasPages())
                <section id="section-pagination">
                    <nav class="pagination-container" aria-label="Pagination Navigation">
                        ...
                        @else
                            <a href="{{ $kosts->previousPageUrl() }}" class="nav-button prev" aria-label="Previous page">
                                <img src="{{ asset('images/arrow-left.svg') }}" alt="Previous" />
                            </a>
                        @endif
                        ...
                        <ul class="page-numbers">
                            @foreach($kosts->getUrlRange(1, $kosts->lastPage()) as $page => $url)
                                @if($page == $kosts->currentPage())
                                    <li><button class="page-number active" aria-current="page">{{ $page }}</button></li>
                                @else
